...
import com.squins.gdx.backends.bytecoder.api.web.event.MouseEvent
import com.squins.gdx.backends.bytecoder.api.web.event.NativeEvent
 ...
 */
class BytecoderInput(val canvas: LibgdxAppCanvas, config: BytecoderApplicationConfiguration) : Input, EventListener<NativeEvent> {
    ...
    private fun addEventListener(target: Node, name: String, handler: EventListener<NativeEvent>) {
    ...
    override fun run(e: NativeEvent) {
        ...
        if (e.type().equals("mousemove")) {
            if (isCursorCatched) {
                this.deltaX[0] = getMovementXJSNI(e);
                this.deltaY[0] = getMovementYJSNI(e);
                this.touchX[0] += getMovementXJSNI(e);
                this.touchY[0] += getMovementYJSNI(e);
            } else {
